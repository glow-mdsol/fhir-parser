...
class FHIRRenderer(object):
    ...
    def __init__(self, spec, settings):
        self.spec = spec
        self.settings = self.__class__.cleaned_settings(settings, spec)
        self.jinjaenv = Environment(loader=PackageLoader('generate', self.settings.tpl_base))
        self.jinjaenv.filters['wordwrap'] = do_wordwrap

    @classmethod
    def cleaned_settings(cls, settings, spec):
        """
        Splits paths at '/' and re-joins them using os.path.join().
        """
        # Root output dir for models
        if settings.target_base:
            target_base = os.path.join(*settings.target_base.split('/'))
        else:
            # default value (relative to Default/settings.py)
            target_base = os.path.join('..', 'models')
        # If we want to include the version in the path
        if settings.versioned_models:
            # rewrite the version to make it python friendly
            target_base = os.path.join(target_base, settings.path_safe_version)
        # target_base is the root for the generated models
        settings.target_base = target_base
        settings.tpl_base = os.path.join(*settings.tpl_base.split('/'))
        # default this
        settings.tpl_init_target = os.path.join(target_base, '__init__.py')
        # use as is if supplied
        if settings.tpl_resource_target:
            settings.tpl_resource_target = os.path.join(*settings.tpl_resource_target.split('/'))
        else:
            settings.tpl_resource_target = target_base
        # use as is if supplied
        if len(settings.tpl_factory_target.split('/')) > 1:
            # have a path reference
            settings.tpl_factory_target = os.path.join(*settings.tpl_factory_target.split('/'))
        else:
            # have a file reference
            settings.tpl_factory_target = os.path.join(target_base, settings.tpl_factory_target)
        # use as is if supplied
        if settings.tpl_unittest_target:
            settings.tpl_unittest_target = os.path.join(*settings.tpl_unittest_target.split('/'))
        else:
            settings.tpl_unittest_target = target_base
        return settings
...
class FHIRModuleRenderer(FHIRRenderer):
    """
    Generate an __init__.py and send the requirements
    """

    def copy_files(self):
        """
        Copy any other requisite files
        :param target_dir:
        :return:
        """
        files = ['requirements.txt']
        for filename in files:
            filepath = os.path.join(self.settings.tpl_base, filename)
            if os.path.exists(filepath):
                tgt = os.path.join(self.settings.target_base, os.path.basename(filepath))
                logger.info("Copying {} to {}".format(os.path.basename(filepath), tgt))
                shutil.copyfile(filepath, tgt)
            else:
                logger.error("Requisite file {} not found".format(filepath))
    ...
    def render(self):
        ...
        self.do_render(data, self.settings.tpl_init_source, self.settings.tpl_init_target)
        self.copy_files()
